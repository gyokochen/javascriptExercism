...
{
  const dayRateWithDiscount = applyDiscount(dayRate(ratePerHour), discount);
  return Math.ceil(dayRateWithDiscount * DAYS_PER_MONTH);
}
...
{
  const dayRateWithDiscount = applyDiscount(dayRate(ratePerHour), discount);
  return Math.floor(budget / dayRateWithDiscount);
}
